module Dec8.ArrayOfList (run) where
...
data Forest = Forest {
  trows :: Array Int [Height],
  tcols :: Array Int [Height]
  }
  deriving Show
...
intoForest rows = Forest (arrayFromList rows) (arrayFromList $ transpose rows)

arrayFromList :: [a] -> Array Int a
arrayFromList xs =
  let
    lowestIdx = 0
    highestIdx = (length xs) - 1
    withIndex = zipWithIndex xs
  in
